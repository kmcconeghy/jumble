...
microbenchmark(
  jumble = mdis_grps(df_mdist[df_mdist$arms==0, 2:length(df_mdist)], 
          df_mdist[df_mdist$arms==1, 2:length(df_mdist)]) * ssc ,
...
```{r, echo=F}
X_delta <- apply(df_mdist, 2, function(x) x - mean(x))[, 2:length(df_mdist)] 
```
...
tst <- df_mdist[, 2:length(df_mdist)] %>% as.matrix(.)
