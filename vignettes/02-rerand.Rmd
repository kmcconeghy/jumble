...
var_rern <- sd(tab_1[1:nrows_vars, 6])
var_pct <- (var_rern / var_tri * 100)
...
for (i in 1:rndms) {
  ...
  sd_iter <- as.integer(as.Date('2019-06-28') + i)
  
  rnd_id <- df$pidnum
  
  df_iter <- bind_cols(df[, vars], rnd_allot(rnd_id, sd_iter)[, 2]) %>%
    as.data.frame(.)
...
rndms <- 1000L
#Matrix of values  
rernd_mn_diff <- matrix(nrow = rndms, ncol = length(vars))  
...
ggplot(md_diff_grp, 
       aes(x=factor(var), y=x, color = random, fill = random)) +
...
Refer to the permutation vignette for implications of this randomization
strategy in the analysis stage.  
